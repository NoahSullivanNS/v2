...
def run_loop():
    while True:
        print(f"[{datetime.utcnow().isoformat()}] Fetching news...")
        articles = fetch_headlines()

        for article in articles:
            title = article.get("title")
            if Joke.objects.filter(headline=title).exists():
                continue  # Skip if joke already exists
            ...
            print(f"Stored: {joke.headline[:60]}")

        print("Sleeping for 15 minutes...\n")
        time.sleep(900)  # Wait 15 minutes before next fetch
